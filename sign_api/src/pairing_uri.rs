...
lazy_static! {
    static ref TOPIC_AND_VERSION_RE: Regex =
        Regex::new(r"^(?P<topic>[[[:word:]]-]+)@(?P<version>\d+)$")
            .expect("invalid TOPIC_AND_VERSION_RE in wallet_connect");
}
